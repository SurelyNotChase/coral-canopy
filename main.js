...
const setupColorTracker = (videoSource, index) => {
    ...
    const myColors = ['yellow','magenta','cyan'];
...
const animate = () => {
    ...
    experience.scene.children.filter((item) => item.type === "Group").forEach((object, index) => {
        collision.detectCollisions(generate, sphere);
        ...
        if (index === 0) {
            ...
            //object.lookAt(0, 0, 0);
           if(object.position.z > 1 || object.position.z < -1) object.position.z = utils.lerp(object.position.z, 0, 0.05);
            
           if(object.position.x > 1 || object.position.x < -1) object.position.x = (utils.lerp(object.position.x, 0, 0.05));
           ...
           if(object.position.x > 1 || object.position.x < -1) object.position.x = (utils.lerp(object.position.x, 0, 0.05));
            generate[index].boundingBox.min.x = -(utils.lerp(object.position.x, 0, 0.01));
            generate[index].boundingBox.max.x = -(utils.lerp(object.position.x, 0, 0.01));
            generate[index].boundingBox.min.y = -(utils.lerp(object.position.y, 0, 0.01));
            generate[index].boundingBox.max.y = -(utils.lerp(object.position.y, 0, 0.01));
            generate[index].boundingBox.min.z = -(utils.lerp(object.position.z, 0, 0.01));
            generate[index].boundingBox.max.z = -(utils.lerp(object.position.z, 0, 0.01));
            //object.rotation.x += (90 * Math.PI) / 180;
...
const mount = () => {
    ...
    const cam1Constraints = {
        ...
        'video': {
            'deviceId': "3afeed588ee200af12fb20d052eb9859f7a1dc4b56fed9d095509d6a9be7bd10",//"def272f22344671423eb09c1a1cfcd6c00ceb0b887e687a43ec0d8b3b78fc3e2",
    ...
    const cam2Constraints = {
        // 'audio': { 'echoCancellation': true },
        'video': {
            'deviceId': "964ce7dc3e69340b75627ccd1d0a4a639a496ca482942ae29bcb671ef1d93513", //"c87a60835c52aa448d4998f7f0c92dba0de0f473dbcc854cd1a4d1f405d4b7aa",
...
const populateScene = async () => {
    ...
    */
    ...
    collision.calculateCollisionPoints(generate, generate[0].meshObject);
...
export default { init, gameObjects, masterAnimations, portalVideos, generate }